...
`;
  ...
  function persistFamilyRelayProfile() {
    try {
      localStorage.setItem(familyRelayProfileKey, JSON.stringify(getFamilyRelayProfile()));
    } catch (err) {
      console.warn("Could not save family relay profile:", err);
    }
  }

  function loadFamilyRelayProfile() {
    try {
      const parsed = JSON.parse(localStorage.getItem(familyRelayProfileKey) || "{}");
  ...
  function loadFamilyRelayRecipients() {
    if (!familyRelayEmailInput) return;
    try {
      const stored = localStorage.getItem(familyRelayRecipientsKey) || "";
  ...
  function persistFamilyRelayRecipients(value) {
    try {
      localStorage.setItem(familyRelayRecipientsKey, String(value || ""));
    } catch (err) {
      console.warn("Could not save family relay recipients:", err);
    }
  }
