document.addEventListener("DOMContentLoaded", () => {
  ...
  function chooseRoleAndRedirect(role) {
    ...
    if (role === "hospital_staff") {
      if (memberOnboardingStatus) {
        memberOnboardingStatus.textContent = getStaticFallbackText("Hospital staff selected. Choose hospital, then Save and Continue.");
      }
      if (memberHospitalSelect) {
        memberHospitalSelect.focus();
      }
      return;
    }
...
`;
  ...
  function buildFhirBundle() {
    return {
      ...
      entry: localLogs.slice(-20).map((item, index) => ({
        fullUrl: `urn:uuid:carevoice-observation-${item.timestamp || index}`,
        resource: {
          resourceType: "Observation",
          status: "preliminary",
          code: { text: `CareVoice voice triage: ${item.category || "general"}` },
          subject: { reference: `Patient/${String(item.userId || activeUserId || "local-demo")}` },
  ...
  function exportFhirBundle() {
    const blob = new Blob([JSON.stringify(buildFhirBundle(), null, 2)], { type: "application/fhir+json;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
  ...
  function exportEhrssReadyPack() {
    const pack = {
      ...
      consent: {
        status: "demo-consent-required-before-real-upload",
