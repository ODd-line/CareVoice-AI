document.addEventListener("DOMContentLoaded", () => {
  ...
  const portalChoosePatientBtn = document.getElementById("portalChoosePatientBtn");
  const portalChooseHospitalStaffBtn = document.getElementById("portalChooseHospitalStaffBtn");
  const portalChooseFamilyMemberBtn = document.getElementById("portalChooseFamilyMemberBtn");
  ...
  function chooseRoleAndRedirect(role) {
    ...
    if (!user) {
      ...
      if (memberOnboardingStatus) {
        memberOnboardingStatus.textContent = getStaticFallbackText(`Sign in with Google to continue as ${getMemberRoleLabel(role)}. CareVoice will keep this choice after login.`);
      }
      showLoginPortal();
      signInWithGoogle();
      return;
  ...
  function applyMemberOnboardingGate(user) {
    ...
    if (memberOnboarding) {
      memberOnboarding.classList.toggle("hidden", fullyComplete && !forceShowMemberOnboarding);
    }
    ...
    if (memberStatusText) {
      if (!isAuthed) {
        memberStatusText.textContent = getStaticFallbackText("Choose a portal below, then sign in with Google to continue.");
...
`;
  ...
  function renderOrgProjectsUI() {
    ...
    const canCreateStaffProject = !!(!active && currentOrgUser && currentMemberProfile && currentMemberProfile.role === "hospital_staff");
    ...
    if (addOrgRoomBtn) addOrgRoomBtn.disabled = !canCreateStaffProject && (!active || !isOwner);
    ...
    if (orgProjectStatus) {
      if (!active) {
        orgProjectStatus.textContent = canCreateStaffProject
          ? "Add your first room and CareVoice will create a personal hospital room board for this account."
          : "Create a project, add members, and set up rooms with watch-outs and custom action buttons.";
      } else {
    ...
    if (!active) {
      ...
      p.textContent = canCreateStaffProject ? "Use Add Room to start your personal hospital board." : "Create or select a project to view rooms.";
  ...
  function buildDefaultStaffProject(user) {
    const hospitalLabel = getHospitalLabelFromProfile(currentMemberProfile) || "CareVoice Ward Team";
    const userEmail = getUserEmail(user);
    return {
      id: `org-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
      name: `${hospitalLabel} Rooms`,
      ownerUid: user.uid,
  ...
  async function ensureStaffOrgProject(user) {
    const active = getActiveOrgProject();
    if (active) return active;
    if (!user || !user.uid || !currentMemberProfile || currentMemberProfile.role !== "hospital_staff") return null;
    const project = buildDefaultStaffProject(user);
    orgProjects.unshift(project);
    setActiveOrgProject(project.id);
    if (orgProjectStatus) {
      orgProjectStatus.textContent = `Personal room board created: ${project.name}. Add Room 1, Room 2, or a patient bed now.`;
    }
    await persistActiveOrgProject();
    return project;
  }
  ...
  async function initOrgProjectsForUser(user) {
    ...
    if (user && currentMemberProfile && currentMemberProfile.role === "hospital_staff" && !orgProjects.length) {
      await ensureStaffOrgProject(user);
    }
  ...
  function setFeatureAccess(isMember) {
    const hideGenericMemberPanels = getCurrentWorkspacePageName() === "member.html" && !isMember;
    ...
    memberLockables.forEach((section) => {
      ...
      section.classList.toggle("hidden", hideGenericMemberPanels);
    });
  ...
  function updateAuthUI(user) {
    if (user) {
      ...
      if (memberAccess) memberAccess.classList.add("hidden");
      loadHospitalAccounts();
      ...
      if (profileEntryPanel && !isMemberWorkspacePage()) {
        ...
        if (savedRoleLabel && isMemberProfileComplete(currentMemberProfile)) {
        ...
        if (profileEntryStatus) {
          ...
          profileEntryStatus.textContent = pendingRoleLabel
      ...
      if (isMemberWorkspacePage()) {
        ...
        if (desiredRoute === "member") {
          clearPostAuthRoute();
          if (!isMemberProfileComplete(currentMemberProfile)) {
            if (profileEntryPanel) {
              profileEntryPanel.classList.remove("hidden");
              glideToElement(profileEntryPanel, 22);
            }
            return;
      ...
      if (memberStatusText) {
        memberStatusText.textContent = isMemberWorkspacePage()
          ? "Sign in with Google first. After login, choose Patient, Hospital Staff, or Family Member."
          : "Trial mode active. Member login required for exports, judge mode, and cloud sync.";
      }
      if (memberNameValue) memberNameValue.textContent = "Not Signed In";
      if (memberModeValue) memberModeValue.textContent = "Trial";
      if (memberAccess) memberAccess.classList.toggle("hidden", isMemberWorkspacePage());
      ...
      if (switchAccountBtn) {
        ...
        const switchText = switchAccountBtn.querySelector("span");
        if (switchText) switchText.textContent = "Sign in with Google";
      }
      ...
      if (loginPortal && isMemberWorkspacePage()) {
        loginPortal.classList.remove("hidden");
        if (authStatusText) {
          authStatusText.textContent = "Sign in with Google. If this account has no saved role, CareVoice will show the three role choices next.";
        }
      }
      if (memberOnboarding && isMemberWorkspacePage()) {
        memberOnboarding.classList.remove("hidden");
        if (memberOnboardingStatus) {
          memberOnboardingStatus.textContent = "Choose Patient, Hospital Staff, or Family Member. CareVoice will ask for Google login and then open that private workspace.";
        }
      }
      if (profileEntryPanel) {
        profileEntryPanel.classList.add("hidden");
  ...
  if (portalChoosePatientBtn) {
    portalChoosePatientBtn.addEventListener("click", () => chooseHomeIdentityAndRedirect("patient"));
  }

  if (portalChooseHospitalStaffBtn) {
    portalChooseHospitalStaffBtn.addEventListener("click", () => chooseHomeIdentityAndRedirect("hospital_staff"));
  }

  if (portalChooseFamilyMemberBtn) {
    portalChooseFamilyMemberBtn.addEventListener("click", () => chooseHomeIdentityAndRedirect("family_member"));
  }
  ...
  if (addOrgRoomBtn) {
    addOrgRoomBtn.addEventListener("click", async () => {
      let active = getActiveOrgProject();
      if (!active && currentOrgUser) {
        active = await ensureStaffOrgProject(currentOrgUser);
        renderOrgProjectsUI();
      }
      if (!active) {
        window.alert("Please sign in as hospital staff to add rooms.");
        return;
      }
      if (!currentOrgUser || active.ownerUid !== currentOrgUser.uid) {
      ...
      const typedName = String(orgRoomNameInput ? orgRoomNameInput.value : "").trim();
      const name = typedName || `Room ${(Array.isArray(active.rooms) ? active.rooms.length : 0) + 1}`;
