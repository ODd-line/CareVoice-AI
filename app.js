document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateAuthUI(user) {
    ...
    if (user) {
      ...
      const desiredRoute = sessionStorage.getItem(postAuthRouteKey);
    ...
    try {
      ...
      if (authStatusText) {
        if (code === "auth/unauthorized-domain") {
          ...
          authStatusText.textContent = "Popup sign-in was blocked. Switching to redirect sign-in...";
        } else {
          authStatusText.textContent = "Google sign-in failed. Check Firebase provider setup and authorized domains.";
        }
      }

      // Fallback: redirect sign-in works better on some browsers and strict privacy settings.
      if (code !== "auth/unauthorized-domain") {
        try {
  ...
  if (googleLoginBtn) {
    googleLoginBtn.addEventListener("click", () => {
      sessionStorage.setItem(postAuthRouteKey, "member");
      signInWithGoogle();
    });
  }
  ...
  if (openRegisterBtn) {
    openRegisterBtn.addEventListener("click", showLoginPortal);
  }
