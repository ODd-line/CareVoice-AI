...
`;
  ...
  function setupNavStickyFallback() {
    ...
    // Only use this workaround for Safari, where `position: sticky` can fail
    // in the presence of certain overflow/layout combinations.
    ...
    const check = () => {
      if (forced) return;
      if ((window.scrollY || 0) < 160) return;
