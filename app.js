document.addEventListener("DOMContentLoaded", () => {
  ...
  async function hydrateHospitalAccessForUser(user) {
    ...
    try {
      const [cloudAccounts, cloudMembership] = await Promise.all([
  ...
  function refreshHospitalAccountUi(user) {
    ...
    if (account) {
      if (hospitalAccountStatus) {
        hospitalAccountStatus.textContent = `Connected to ${account.name}. Share join code ${account.code} with your hospital team.`;
      }
      if (hospitalAccountCodeHint) {
        hospitalAccountCodeHint.textContent = `Current account: ${account.name} • Join code: ${account.code}`;
      }
    } else {
      if (hospitalAccountStatus) {
  ...
  function saveMemberProfileForUser(user) {
    ...
    if (!isMemberProfileComplete(profile)) {
      if (memberOnboardingStatus) {
        memberOnboardingStatus.textContent = "Please choose role and hospital. If hospital is Other, enter its name.";
      }
      return false;
    }
    ...
    try {
      ...
      if (memberOnboardingStatus) {
        memberOnboardingStatus.textContent = "Profile saved. Member workspace unlocked.";
      }
      return true;
    } catch (err) {
      console.warn("Could not save member onboarding profile:", err);
      if (memberOnboardingStatus) {
        memberOnboardingStatus.textContent = "Could not save profile. Please try again.";
      }
      return false;
    }
  }
  ...
  function applyMemberOnboardingGate(user) {
    ...
    if (memberOnboarding) {
      memberOnboarding.classList.toggle("hidden", !isAuthed || fullyComplete);
    }

    const shouldUnlock = isAuthed && fullyComplete;
    ...
    if (memberStatusText) {
      if (!isAuthed) {
        memberStatusText.textContent = "Guest mode active. Please sign in.";
      } else if (!profileComplete) {
        memberStatusText.textContent = "Signed in. Complete role and hospital setup to unlock the workspace.";
      } else if (requiresHospitalAccount && !hasHospitalAccount) {
        memberStatusText.textContent = "Signed in. Hospital staff must create or join a hospital account before entering the workspace.";
      }
    }

    if (memberOnboardingStatus && isAuthed && profileComplete && requiresHospitalAccount && !hasHospitalAccount) {
      memberOnboardingStatus.textContent = "Hospital staff need a hospital account. Create one or join with a code below.";
    }
  ...
  function routeToRoleWorkspaceIfNeeded(user, force = false) {
    ...
    if (role === "hospital_staff") {
      const hasHospitalAccount = Boolean(currentHospitalMembership && currentHospitalMembership.accountId && getHospitalAccountById(currentHospitalMembership.accountId));
      if (!hasHospitalAccount) return false;
    }
...
`;
  ...
  function updateAuthUI(user) {
    if (user) {
      ...
      if (isMemberWorkspacePage()) {
        ...
        // Signed-in users should never stay on the public home page.
        // Route to onboarding first, then to the role workspace once setup is complete.
        if (!currentMemberProfile || !isMemberProfileComplete(currentMemberProfile)) {
        ...
        const roleTarget = getRoleWorkspaceRoute(currentMemberProfile.role);
        if (getCurrentWorkspacePageName() !== roleTarget) {
  ...
  if (saveMemberProfileBtn) {
    saveMemberProfileBtn.addEventListener("click", () => {
      ...
      if (!user) {
        if (memberOnboardingStatus) {
          memberOnboardingStatus.textContent = "Please sign in before saving profile.";
        }
        return;
      }
  ...
  if (createHospitalAccountBtn) {
    createHospitalAccountBtn.addEventListener("click", async () => {
      const user = window.auth && window.auth.currentUser ? window.auth.currentUser : null;
      if (!user || !user.uid) {
        if (hospitalAccountStatus) hospitalAccountStatus.textContent = "Sign in first to create a hospital account.";
        return;
      }

      if (!currentMemberProfile || String(currentMemberProfile.role || "") !== "hospital_staff") {
        if (hospitalAccountStatus) hospitalAccountStatus.textContent = "Only hospital staff roles can create hospital accounts.";
        return;
      }
      ...
      if (!accountName) {
        if (hospitalAccountStatus) hospitalAccountStatus.textContent = "Enter a hospital account name first.";
        return;
      }
      ...
      const account = {
        ...
        admins: [user.uid],
        adminEmails: [getUserEmail(user)],
        members: [user.uid],
        memberEmails: [getUserEmail(user)],
        createdAt: Date.now()
      };
  ...
  if (joinHospitalAccountBtn) {
    joinHospitalAccountBtn.addEventListener("click", async () => {
      const user = window.auth && window.auth.currentUser ? window.auth.currentUser : null;
      if (!user || !user.uid) {
        if (hospitalAccountStatus) hospitalAccountStatus.textContent = "Sign in first to join a hospital account.";
        return;
      }

      if (!currentMemberProfile || String(currentMemberProfile.role || "") !== "hospital_staff") {
        if (hospitalAccountStatus) hospitalAccountStatus.textContent = "Only hospital staff roles can join hospital accounts.";
        return;
      }

      const code = String(joinHospitalAccountCodeInput ? joinHospitalAccountCodeInput.value : "").trim().toUpperCase();
      if (!code) {
        if (hospitalAccountStatus) hospitalAccountStatus.textContent = "Enter a join code first.";
        return;
      }
      ...
      if (!account) {
        if (hospitalAccountStatus) hospitalAccountStatus.textContent = "Join code not found. Check the code and try again.";
        return;
      }
  ...
  if (hospitalInviteBtn) {
    hospitalInviteBtn.addEventListener("click", async () => {
      ...
      if (!user || !user.uid || !currentHospitalMembership || !currentHospitalMembership.accountId) {
        if (hospitalAccountStatus) hospitalAccountStatus.textContent = "Join a hospital account first.";
        return;
      }

      const account = getHospitalAccountById(currentHospitalMembership.accountId);
      if (!account || !canManageHospitalAccount(user, account)) {
        if (hospitalAccountStatus) hospitalAccountStatus.textContent = "Only account owner/admin can add staff emails.";
        return;
      }

      const email = String(hospitalInviteEmailInput ? hospitalInviteEmailInput.value : "").trim().toLowerCase();
      if (!email || !email.includes("@")) {
        if (hospitalAccountStatus) hospitalAccountStatus.textContent = "Enter a valid staff email.";
        return;
      }
  ...
  if (hospitalAccountMembersList) {
    hospitalAccountMembersList.addEventListener("click", async (event) => {
      ...
      if (ownerEmail && ownerEmail === String(email).toLowerCase()) {
        if (hospitalAccountStatus) hospitalAccountStatus.textContent = "Owner email cannot be removed.";
        return;
      }
  ...
  function analyzeInput(text) {
    ...
    if (aiSignalSeverity) aiSignalSeverity.textContent = familyRelayHit ? "Medium" : severity;
    if (aiSignalNextStep) {
      ...
      if (familyRelayHit && requestedChannel === "whatsapp") {
        ...
        aiSignalNextStep.textContent = familyRelayHit ? "Prepare a family update and copy the message." : nextStep;
      }
    }
